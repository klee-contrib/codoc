...
disponible et, si oui, l'installe automatiquement (`npm install -g codoc-cli@<version>`) avant de
relancer la commande d'origine avec le binaire à jour. Jamais en CI (`CI` défini) ni en sortie
non-interactive (`isTTY` faux).

Pour désactiver complètement cette vérification, ajoute `autoUpdate: false` dans `codoc.yaml`.
...
fin de commande). Puis les identifiants (mêmes conventions que `.env-codoc` ci-dessous, avec la même
offre de sauvegarde).
